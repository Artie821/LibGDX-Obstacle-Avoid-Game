...
public class DebugCameraConfig {
    ...
    public static final Logger log = new Logger(DebugCameraController.class.getName(), Logger.DEBUG);
    ...
    private void init() {
        ...
        if (fileHandle.exists()) {
            ...
            log.debug("Using defaults, file does not exist " + FILE_PATH);
            setupDefaults();
        }
    }
    ...
    private void load() {
        try {
            ...
            log.error("Error loading file " + FILE_PATH + " using defaults", e);
            setupDefaults();
        }
    }

    private void setupDefaults() {
        maxZoomIn = DEFAULT_MAX_ZOOM_OUT;
    ...
    private static int getInputKeyValue(JsonValue root, String name, int defaultValueKey) {
        String keyString = root.getString(name, Input.Keys.toString(defaultValueKey));
